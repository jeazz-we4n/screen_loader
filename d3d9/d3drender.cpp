...
HRESULT CD3DFont::Print( const char* text, SRColor color, float x, float y, bool skipColorTags, bool noColorFormat )
{
...
HRESULT CD3DFont::PrintShadow (float x, float y, SRColor color, const char* text )
{
    if (color.red >= 0x80 && color.green >= 0x80 && color.blue >= 0x80)
        Print(text, SRColor(eCdBlack, color.alpha), x + 1, y + 1, true, false);
    else
        Print(text, SRColor(eCdWhite, color.alpha), x + 1, y + 1, true, false);
    return Print(text, color, x, y, false, false);
}

float CD3DFont::DrawLength ( const char* text, bool noColorFormat ) const
{
    float	len = 0.0f;
    float	sub = ( m_dwCreateFlags & FCR_BORDER ) ? 2.0f : 0.0f;
    if (!text) return 0.0f;
    size_t strl = strlen(text);
    for ( size_t cpos = 0; cpos < strl && text[cpos] != '\0'; cpos++ )
    {
        int c = ( byte ) ( text[cpos] ) - 32;
        if ( !( c >= 0 && c < 224 ) )
            continue;
...
size_t CD3DFont::GetCharPos( const char* text, float x, bool noColorFormat ) const
{
    ...
    return pos;
             ...
             if (SUCCEEDED(Begin(D3DPT_LINELIST)))
        {
             ...
             D3DVertex2f(static_cast<float>(x2), static_cast<float>(y2));
             End();
    }
    }
