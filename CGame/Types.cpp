...
float RwV3D::length()
{
    return static_cast<float>(sqrt(pow(fX, 2) + pow(fY, 2) + pow(fZ, 2)));
}
...
float RwV3D::angle(float X, float Y)
{
    ...
    float a = static_cast<float>(180 * atan((float)t) / M_PI);
...
RwMatrix RwMatrix::rotate(const RwV3D *param, float theta)
{
    ...
    // rotate X
    mRotateMult.right.fX = static_cast<float>(cos_t + ( 1.0f - cos_t ) * param->fX * param->fX);
    mRotateMult.right.fY = static_cast<float>(( 1.0f - cos_t ) * param->fX * param->fY - sin_t * param->fZ);
    mRotateMult.right.fZ = static_cast<float>(( 1.0f - cos_t ) * param->fX * param->fZ + sin_t * param->fY);
    // rotate Y
    mRotateMult.at.fX = static_cast<float>(( 1.0f - cos_t ) * param->fY * param->fX + sin_t * param->fZ);
    mRotateMult.at.fY = static_cast<float>(cos_t + ( 1.0f - cos_t ) * param->fY * param->fY);
    mRotateMult.at.fZ = static_cast<float>(( 1.0f - cos_t ) * param->fY * param->fZ - sin_t * param->fX);
    // rotate Z
    mRotateMult.up.fX = static_cast<float>(( 1.0f - cos_t ) * param->fZ * param->fX - sin_t * param->fY);
    mRotateMult.up.fY = static_cast<float>(( 1.0f - cos_t ) * param->fZ * param->fY + sin_t * param->fX);
    mRotateMult.up.fZ = static_cast<float>(cos_t + ( 1.0f - cos_t ) * param->fZ * param->fZ);
    // multiply matrix
